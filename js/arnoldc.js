...
function evalScripts() {
    ...
    _.filter(document.getElementsByTagName('script'), function(script) {
        ...
        script.insertAdjacentHTML('afterend', '<pre>' + execArnoldC(script.text) + '</pre>');
    });
}
    ...
    function stat(statement) {
        ...
        if (result && result.index === 0) {
            ...
            return {
                ...
                match: _.head(program.match(re)),
                index: result.index,
                arguments: _.tail(program.match(re))
            };
        } else {
          return null;
...
function evalTextArea() {
    ...
    var output = execArnoldC(src);
    document.getElementById('output').innerText = output;
    document.getElementById('js').innerText = transpileArnoldC(src);
}
